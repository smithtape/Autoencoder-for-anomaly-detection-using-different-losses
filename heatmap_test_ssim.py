...
model.load_state_dict(torch.load('/home/smithtape/Desktop/phd_ncu_ubuntu/20230917_lab1/code3/autoencoder_ssim.pth', map_location=device))
model.eval()

with open('ssim_heatmap_results.txt', 'w') as f:
    f.write("Filename\tSSIM Loss\tAnomaly Status\tCategory\n")
    for idx, (img, label) in enumerate(test_loader):
        img = img.to(device)
        output = model(img)
        loss = 1 - ssim(img, output)  # Using 1-ssim as the loss
        ...
        cv2.imwrite(f'heatmap_images_ssim/{img_name}_heatmap.png', heatmap)
        
        category = "fault" if label.item() == 0 else "non_fault"
        anomaly_status = "Normal" if loss < 0.04 else "Anomaly"  # Threshold might need adjustment
