...
model.load_state_dict(torch.load('/home/smithtape/Desktop/phd_ncu_ubuntu/20230917_lab1/code3/autoencoder_mse.pth', map_location=device))
model.eval()

with open('mse_heatmap_results.txt', 'w') as f:
    f.write("Filename\tMSE Loss\tAnomaly Status\tCategory\n")
    for idx, (img, label) in enumerate(test_loader):
        img = img.to(device)
        output = model(img)
        loss = torch.nn.functional.mse_loss(output, img)
        ...
        cv2.imwrite(f'heatmap_images_mse/{img_name}_heatmap.png', heatmap)
